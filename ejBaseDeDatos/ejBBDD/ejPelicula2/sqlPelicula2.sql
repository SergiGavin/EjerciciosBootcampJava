...
-- 1. Todos los campos de directores.

SELECT * FROM directors;
-- 2. Añadir alias al nombre
SELECT name as nombre_del_director FROM directors;
-- 3. Ordenar tabla
SELECT * FROM directors order by name;
-- 4 Todas las pelis que tengan :
SELECT * FROM movies WHERE title LIKE '%:%';
-- 5 . Actores que empiecen por M
...
-- 13
SELECT * FROM movies WHERE id <= 3 or id >= 6 ;
...
-- 16
SELECT * FROM movies WHERE year != 2013 and year != 2014 and year != 1994 ;
-- 17
...
-- 26
SELECT round(SUM(metascore)/10,1) as 'RATING_POINTS' FROM movies;
-- 27
SELECT * FROM movies where metascore > (SELECT avg(metascore)FROM movies);
-- 28
SELECT * FROM movies where id not between 2 and 4;
-- 29
SELECT m.title AS 'Título de la Película', d.name AS 'Director'
FROM movies AS m
JOIN movies_directors AS md ON m.id = md.movies_id
JOIN directors AS d ON md.directors_id = d.id;

-- 30
SELECT m.title AS 'Título de la Película', s.name AS 'Actor'
FROM movies AS m
JOIN movies_stars AS ms ON m.id = ms.movies_id
JOIN stars AS s ON ms.stars_id = s.id where m.title = 'Pulp Fiction' ;

-- 31
SELECT m.title AS 'Título', g.name AS 'GENERO'
FROM movies AS m
JOIN movies_genres AS mg ON m.id = mg.movies_id
JOIN genres AS g ON mg.genres_id = g.id;
-- 32
SELECT m.title AS 'Título', g.name AS 'GENERO', s.name AS 'ACTOR', d.name AS 'DIRECTOR'
FROM movies AS m
JOIN movies_genres AS mg ON m.id = mg.movies_id
JOIN genres AS g ON mg.genres_id = g.id
JOIN movies_stars AS ms ON m.id = ms.movies_id
JOIN stars AS s ON ms.stars_id = s.id
JOIN movies_directors AS md ON m.id = md.movies_id
JOIN directors AS d ON md.directors_id = d.id;

-- 33
SELECT m.title AS 'Título', count(g.id) AS 'GENRES'
FROM movies AS m
JOIN movies_genres AS mg ON m.id = mg.movies_id
JOIN genres AS g ON mg.genres_id = g.id group by m.id, m.title;

-- 34
SELECT d.name AS 'DIRECTOR', count(m.id) AS 'TOTAL_PELICULAS'
FROM directors AS d
JOIN movies_directors AS md ON d.id = md.directors_id
JOIN movies AS m ON md.movies_id = m.id group by d.id, d.name;

-- 35
SELECT title as 'TITLE', CASE
        WHEN metascore >= 90 THEN 'EXCELENTE'
        WHEN metascore >= 70 THEN 'NOTABLE'
        WHEN metascore >= 60 THEN 'BIEN'
        WHEN metascore >= 50 THEN 'SUFICIENTE'
        ELSE 'INSUFICIENTE'
    END AS 'VALORACIÓN' from movies;

-- 36
INSERT INTO 
	movies (id, title, year, image_url, certificate, runtime, imdb_rating, description, metascore, votes, gross) 
VALUES
	(NULL, 'CREPUSCULO', 2008, 'url_crepusculo','NO', 1, 3.2, 'Va sobre 1 vampiro, 1 hombre-lobo y una esquizofrenica',30, 50000, 1);
